...
import com.example.productapi.repository.FavoriteRepository;
...
public class ProductController{
    ...
    // Vitrin sekmeleri için ürün sayıları:
    //   { "total": 5, "byCategory": { "T-Shirts": 3, "Accessories": 2 } }
    //
    // "total" ayrı sorulur, byCategory'nin toplamı alınmaz: kategorisi boş
    // (null) olan ürünler hiçbir gruba girmez ama "Ürünler" sekmesinde sayılmalı.
    //
    // Adres "/{id}" ile çakışmaz: Spring sabit yolu ("/category-counts")
    // değişkenli yola ("/{id}") tercih eder.
    @GetMapping("/category-counts")
    public Map<String, Object> getCategoryCounts() {
        Map<String, Long> byCategory = new HashMap<>();
        for (ProductRepository.CategoryCount row : repository.countProductsPerCategory()) {
            // JSON'da null anahtar olamaz; kategorisiz ürünler zaten "total"de sayılıyor.
            if (row.getCategory() != null) {
                byCategory.put(row.getCategory(), row.getProductCount());
            }
        }
        return Map.of("total", repository.count(), "byCategory", byCategory);
    }
